...
const path = process.argv[2] || "sim_results/draft_training.json";
...
const expectedPair = (a, b) =>
	sigmoid(logitPrior + (logit(rate[a]) - logitPrior) + (logit(rate[b]) - logitPrior));
...
console.log(`\n${path}`);
console.log(
	`  ${d.Perspectives} deck-games  |  base win rate ${pct(100 * prior)}  |  ` +
		`${d.Cards.length} cards, ${d.Pairs.length} pairs`
);
console.log(
	`  P(card drawn) ${cardDraw.toFixed(3)}  P(pair drawn) ${pairDraw.toFixed(3)}  ` +
		`ratio ${pairDrawRatio.toFixed(3)}`
);
...
function printPairs(title, rows) {
	console.log(title);
	console.log(
		`  ${pad("Pair", 46)}${pad("Synergy", 10)}${pad("Pair WR", 10)}` +
			`${pad("Expected", 10)}${pad("Raw WR", 9)}n`
	);
	for (const r of rows) {
		console.log(
...
for (const r of byRaw) {
	console.log(
		`  ${pad(r.name, 46)}${pad(pct(r.actual), 10)}synergy ${r.synergy >= 0 ? "+" : ""}` +
			`${r.synergy.toFixed(2)}`
	);
}
console.log();
...
for (const c of [...cards.slice(0, 10), null, ...cards.slice(-10)]) {
	...
	console.log(
		`  ${pad(c.name, 30)}${pad(pct(c.rate), 9)}` +
			`${pad(c.draw === null ? "n/a" : c.draw.toFixed(3), 11)}${c.n}`
	);
}
console.log();
